...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://akatoyo.com"; 
...
export default async function CategoriaPage({ params }: Props) {
  ...
  const productos: ProductoBase[] = (data as ProductoBase[]) || [];
              ...
              <Link key={p.id} href={`/producto/${p.id}`} className="group">
                <div className="border border-zinc-800 rounded-xl p-4 bg-zinc-900/50 hover:border-zinc-600 transition-all h-full flex flex-col">
                  <div className="w-full aspect-square bg-zinc-800 rounded-lg overflow-hidden mb-4">
                    <img src={p.imagen_url} alt={p.nombre} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  </div>
                  ...
                  <div className="mt-4 pt-2 border-t border-zinc-800">
                    <strong className="text-lg text-white">${Number(p.precio).toLocaleString()}</strong>
                  </div>
                </div>
              </Link>
            ))}
